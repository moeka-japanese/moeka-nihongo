...
function renderVideos(){const entries=window.VIDEOS[category][level];if(category==='listening'&&level==='podcast'){$('content').className='grid podcast-grid';$('count').textContent=`${entries.length} videos / 个视频`;$('hint').textContent='';$('content').innerHTML=entries.map(e=>`<article class="podcast-card"><h3>${jp(e.title)}</h3><a href="https://www.youtube.com/watch?v=${encodeURIComponent(e.youtubeId)}" target="_blank" rel="noopener noreferrer">YouTube ↗</a></article>`).join('');return;}$('count').textContent=`${entries.length} videos / 个视频`;$('hint').textContent='Click a thumbnail to play / 点击缩略图播放';$('content').innerHTML=entries.length?entries.map(videoCard).join(''):`<div class="video-empty"><span class="big-kana" aria-hidden="true">${categories[category].icon}</span><h3>動画は準備中です</h3><p>Videos are coming soon.<br>视频正在准备中。</p><a class="channel-link" href="${esc(config.youtubeChannel)}" target="_blank" rel="noopener noreferrer">MOEKA on YouTube ↗</a></div>`;}
const partsOfSpeech={
...
document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(category==='grammar'&&level==='N5')window.BasicPattern.handle(b,speak,stop);if(['pos','kana'].includes(b.dataset.vocabSort)){stop();setPrefs('vocabSort',b.dataset.vocabSort);}if(b.dataset.readingAudio!==undefined)playReadingLesson(b.dataset.readingAudio);if(b.hasAttribute('data-reading-stop'))stop();if(b.dataset.guideLanguage!==undefined)selectGuideLanguage(b.dataset.guideLanguage);if(b.dataset.pronunciationLanguage!==undefined)selectPronunciationLanguage(b.dataset.pronunciationLanguage);if(b.dataset.articleTranslation!==undefined)toggleArticleTranslation(b);if(b.dataset.level)navigate(category,b.dataset.level);if(b.dataset.playVideo!==undefined)playVideo(b);if(b.dataset.entry!==undefined)showCharacter(Number(b.dataset.entry));if(b.dataset.kanjiReading!==undefined&&category==='characters'){e.preventDefault();speak(b.dataset.kanjiReading);}if(b.dataset.listen!==undefined){e.preventDefault();const entry=window.LESSONS[category]?.[level]?.[Number(b.dataset.listen)];if(entry)speak(category==='grammar'&&entry[0]==='AはBです'&&b.dataset.part==='word'?'エーはビーです。':entry[b.dataset.part==='example'?4:0]);}if(b.hasAttribute('data-retry-posts')){loadPosts(true);render(false);}});
