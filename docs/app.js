...
function renderLessons(){const entries=window.LESSONS[category][level];$('count').textContent=`${entries.length} lessons / 项`;$('hint').textContent=category==='characters'?'Click a card to listen / 点击卡片听发音':'Click to expand / 点击展开';
 ...
 $('content').innerHTML=entries.map((e,i)=>`<details class="lesson-accordion" name="lesson-${category}"><summary><span class="lesson-word">${jp(e[0])}</span><span class="lesson-translation"><span lang="en">${esc(e[1])}</span><span lang="zh-Hans">${esc(e[2])}</span><button type="button" class="play-button lesson-listen" data-listen="${i}" data-part="word" aria-label="${esc(plain(e[0]))}: Listen / 听发音">♪ Listen / 听发音</button></span><span class="accordion-mark" aria-hidden="true">＋</span></summary>${lessonBody(e,i)}</details>`).join('');
}
...
document.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.dataset.level)navigate(category,b.dataset.level);if(b.dataset.playVideo!==undefined)playVideo(b);if(b.dataset.entry!==undefined)showCharacter(Number(b.dataset.entry));if(b.dataset.listen!==undefined){e.preventDefault();const entry=window.LESSONS[category]?.[level]?.[Number(b.dataset.listen)];if(entry)speak(entry[b.dataset.part==='example'?4:0]);}if(b.hasAttribute('data-retry-posts')){loadPosts(true);render(false);}});
