...
- 漢字：N5は指定の54字を6見出しで表示。四角いカードを開くと音読み・訓読み、英語・中国語の意味が表示され、各読みを音声再生できます。N4〜N1は従来のサンプル（URL `#characters`）。
...
`docs/kanji.js` の `KANJI_GROUPS.N5` に見出しと漢字を保存しています。見出しは数字・曜日・時間・場所・方角・人・体（場所・方角は1見出し）の6つ。重複していた「人」は1回だけ掲載。各項目は `kanji`, `on`, `kun`, `en`, `zh`。読み中の `.` は送り仮名の区切りで、画面と音声では除去します。音読み・訓読みの主なものを掲載し、名乗りや熟字訓の網羅はしていません。
